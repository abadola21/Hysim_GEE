// Line 176: To customize the area size , adjust the value passed to the .buffer() method.
// Current setting is for a 1km x 1km area, using a buffer of 500 meters: var square = point.buffer(500).bounds();
// Example - To change this to a 10km x 10km area, modify the line as follows:
// Change Line 176 from: var square = point.buffer(500).bounds();
// To: var square = point.buffer(5000).bounds(); This changes the buffer to 5000 meters, adjusting the area to 10km x 10km.
...
var downloadLinkRaw = ui.Label({
      value:'Sentinel Image will be exported to Drive',
      style:{
        color: 'grey',
        backgroundColor:'#D4F1F4'
...
var downloadLinkProcessed = ui.Label({
  value : 'Hyperspectral Image will be exported to Drive',
  style:{
        color: 'grey',
        backgroundColor:'#D4F1F4'
...
var label = ui.Label({
  value: 'Select Visualization to Export',
  style: {
    backgroundColor:'#D4F1F4',
    color: 'grey',
  }
...
var downloadButton = ui.Button('Export to Drive');
...
function create1kmSquare(geom) {
  var center = geom.centroid(1).coordinates();
  var point = ee.Geometry.Point(center);
  var square = point.buffer(5000).bounds(); // Change the boundary to 5000 to obtain hyperspectral image for 10km by 10km
  return square;
}
...
function onDraw(geometry) {
  ...
  // Create a fixed 1km by 1km square based on the centroid
  var fixedSquare = create1kmSquare(geom);
  ...
  imageDropdown.onChange(function(selectedDate) {
    ...
    Export.image.toDrive({
      image: selectedImage,
      description: 'Sentinel_image_' + selectedDate,
      scale: 10,
      region: fixedSquare,
      fileFormat: 'GeoTIFF'
    });
...
// Create visualization parameters for different layers
    ...
    function addDownloadLinks() {
      ...
      if (checkbox2.getValue()) {
        ...
        Export.image.toDrive({
        image: selected_rgb_img,
        description: rgbFileName,
        scale: 10, 
        region: fixedSquare,
        fileFormat: 'GeoTIFF'
      ...
      if (checkbox3.getValue()) {
          ...
          // Export to Drive
          Export.image.toDrive({
            image: selected_fcc_img,
            description: fccFileName,
            scale: 10,
            region: fixedSquare,
            fileFormat: 'GeoTIFF'
    ...
    Export.image.toDrive({
      ...
      description: 'image_' + selectedDate,
      scale: 10,
      // filePerBand:false,
      region: fixedSquare,
      fileFormat: 'GeoTIFF'
...
// Create a function to update the inspector
  ...
  // Add the pixel values to the panel and create a chart
      ...
      var chart = ui.Chart.array.values(bandValues, 0, bandNames)
